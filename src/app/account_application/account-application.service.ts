...
export class AccountApplicationService {
  constructor(private readonly prisma: PrismaService, private readonly events: LeadEventService) {}
  ...
  async create(input: {
    ...
    const app = await this.prisma.accountApplication.create({
      data: {
        ...
        investmentPreferences: this.parseJson(input.investmentPreferencesJson) as Prisma.InputJsonValue,
  ...
  async updateApplicationStatus(applicationId: string, status: ApplicationStatusEnum, remark?: string | null, authorId?: string | null) {
  ...
  async updateKycStatus(applicationId: string, kyc: KycStatusEnum, remark?: string | null, authorId?: string | null) {
    const prev = await this.prisma.accountApplication.findUnique({ where: { id: applicationId } });
    if (!prev) throw new Error('Application not found');
    const next = await this.prisma.accountApplication.update({
      where: { id: applicationId },
  ...
  async findByLead(leadId: string) {
    return this.prisma.accountApplication.findMany({ where: { leadId }, orderBy: { createdAt: 'desc' } });
  }
}
